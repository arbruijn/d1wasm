...
object *object_create_debris(object *parent, int subobj_num)
{
	...
	vm_vec_scale(&obj->mtype.phys_info.velocity,i2f(10 + (30 * psrand() / PSRAND_MAX)));
...
int choose_drop_segment(void)
{
	int	pnum = 0;
	int	segnum = -1;
	int	initial_drop_depth = BASE_NET_DROP_DEPTH + ((psrand() * 10) >> 15);
...
//--old-- 		segnum = (psrand() * Highest_segment_index) >> 15;
	...
	switch (objp->contains_type) {
		case OBJ_POWERUP:
			for (count=0; count<objp->contains_count; count++) {
				...
				new_velocity.x += fixmul(old_mag+F1_0*32, psrand()*rand_scale - 16384*rand_scale);
				new_velocity.y += fixmul(old_mag+F1_0*32, psrand()*rand_scale - 16384*rand_scale);
				new_velocity.z += fixmul(old_mag+F1_0*32, psrand()*rand_scale - 16384*rand_scale);
				...
				new_pos = objp->pos;
//				new_pos.x += (psrand()-16384)*8;
//				new_pos.y += (psrand()-16384)*8;
//				new_pos.z += (psrand()-16384)*8;
...
//				mprintf(0, "                             pos = x=%d y=%d z=%d\n", obj->pos.x, obj->pos.y, obj->pos.z);
				...
				switch (obj->id) {
					...
					default:
//						if (Game_mode & GM_MULTI)
//							obj->lifeleft = (psrand() + F1_0*3) * 64;		//	Lives for 5 to 5.5 binary minutes (a binary minute is 64 seconds)
						break;
				}
			}
			break;
...
//				new_pos.x += (psrand()-16384)*8;
//				new_pos.y += (psrand()-16384)*7;
//				new_pos.z += (psrand()-16384)*6;
