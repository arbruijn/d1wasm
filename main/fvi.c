...
//inline int oflow_check(fix a,fix b) { return (((int64_t)a * b) >> 47) != 0; }
#define oflow_check(a,b) 0
...
//$$
//$$#pragma aux get_sphere_int_dist parm [esi] [ebx] [ecx] value [eax] modify exact [eax ebx ecx edx] = \x
//$$	"mov eax,ebx"		\x
//$$	"imul eax"			\x
//$$							\x
//$$	"mov ebx,eax"		\x
...
//--unused-- #pragma aux mag2d parm [esi] value [eax] modify exact [eax ebx ecx edx] = \x
//--unused-- 	"mov	eax,[esi]"		\x
//--unused-- 	"imul	eax"				\x
//--unused-- 	"mov	ebx,eax"			\x
//--unused-- 	"mov	ecx,edx"			\x
//--unused-- 	"mov	eax,4[esi]"		\x
//--unused-- 	"imul	eax"				\x
//--unused-- 	"add	eax,ebx"			\x
//--unused-- 	"adc	edx,ecx"			\x
