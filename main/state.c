...
#ifndef SHAREWARE
extern int ai_save_state( BFILE * fp );
extern int ai_restore_state( BFILE * fp );
#endif
...
int state_get_save_file(char * fname, char * dsc, int multi )
{
	...
	for (i=0;i<NUM_SAVES; i++ )	{
		...
		valid = 0;
		fp = bfopen( filename[i], "rb" );
		if ( fp ) {
			//Read id
			bfread( id, sizeof(char)*4, 1, fp );
			if ( !memcmp( id, dgss_id, 4 )) {
				//Read version
				bfread( &version, sizeof(int), 1, fp );
				if (version >= STATE_COMPATIBLE_VERSION)	{
					// Read description
					bfread( desc[i], sizeof(char)*DESC_LENGTH, 1, fp );
					//rpad_string( desc[i], DESC_LENGTH-1 );
					// Read thumbnail
					//sc_bmp[i] = gr_create_bitmap(THUMBNAIL_W,THUMBNAIL_H );
					//bfread( sc_bmp[i]->bm_data, THUMBNAIL_W * THUMBNAIL_H, 1, fp );
					valid = 1;
				}
			} 
			bfclose(fp);
                        ...
                        int sg_secondary_weapon, int sg_next_level_num  	)
{
	...
	fp = bfopen( filename, "wb" );
	if ( !fp ) return 0;

//Save id
	bfwrite( dgss_id, sizeof(char)*4, 1, fp );

//Save version
	temp_int = STATE_VERSION;
	bfwrite( &temp_int, sizeof(int), 1, fp );

//Save description
	strncpy( desc, sg_name, DESC_LENGTH );
	bfwrite( desc, sizeof(char)*DESC_LENGTH, 1, fp );
...
// Save the current screen shot...
	...
	if ( cnv )	{
		...
		bfwrite( cnv->cv_bitmap.bm_data, THUMBNAIL_W*THUMBNAIL_H, 1, fp );
		gr_free_canvas( cnv );
	} else {
	 	ubyte color = 0;
	 	for ( i=0; i<THUMBNAIL_W*THUMBNAIL_H; i++ )
	 		bfwrite( &color, sizeof(ubyte), 1, fp );		
	}
...
// Save the mission info...
	bfwrite( &Mission_list[0], sizeof(char)*9, 1, fp );
...
//Save GameTime
	temp_int = 0;
	bfwrite( &temp_int, sizeof(fix), 1, fp );

//Save player info
	bfwrite( sg_player, sizeof(player), 1, fp );

// Save the current weapon info
	temp_byte = sg_primary_weapon;
	bfwrite( &temp_byte, sizeof(byte), 1, fp );
...
// Save the Cheats_enabled
	...
	bfwrite( &state_game_id, sizeof(uint), 1, fp );
	bfwrite( &Laser_rapid_fire, sizeof(int), 1, fp );
	bfwrite( &Ugly_robot_cheat, sizeof(int), 1, fp );
	bfwrite( &Ugly_robot_texture, sizeof(int), 1, fp );
	bfwrite( &Physics_cheat_flag, sizeof(int), 1, fp );
	bfwrite( &Lunacy, sizeof(int), 1, fp );
...
// Save the Between levels flag...
	bfwrite( &between_levels, sizeof(int), 1, fp );

// Save the mission info...
	bfwrite( &Mission_list[Current_mission_num], sizeof(char)*9, 1, fp );

//Save level info
	bfwrite( &Current_level_num, sizeof(int), 1, fp );
	bfwrite( &Next_level_num, sizeof(int), 1, fp );

//Save GameTime
	bfwrite( &GameTime, sizeof(fix), 1, fp );

//Save player info
	bfwrite( &Players[Player_num], sizeof(player), 1, fp );

// Save the current weapon info
	bfwrite( &Primary_weapon, sizeof(byte), 1, fp );
	bfwrite( &Secondary_weapon, sizeof(byte), 1, fp );

// Save the difficulty level
	bfwrite( &Difficulty_level, sizeof(int), 1, fp );

// Save the Cheats_enabled
	bfwrite( &Cheats_enabled, sizeof(int), 1, fp );
	bfwrite( &Game_turbo_mode, sizeof(int), 1, fp );
	...
	//Save object info
		...
		bfwrite( &i, sizeof(int), 1, fp );
		bfwrite( Objects, sizeof(object)*i, 1, fp );
		
	//Save wall info
		i = Num_walls;
		bfwrite( &i, sizeof(int), 1, fp );
		bfwrite( Walls, sizeof(wall)*i, 1, fp );
	
	//Save door info
		i = Num_open_doors;
		bfwrite( &i, sizeof(int), 1, fp );
		bfwrite( ActiveDoors, sizeof(active_door)*i, 1, fp );
	
	//Save trigger info
		bfwrite( &Num_triggers, sizeof(int), 1, fp );
		bfwrite( Triggers, sizeof(trigger)*Num_triggers, 1, fp );
	
	//Save tmap info
		for (i=0; i<=Highest_segment_index; i++ )	{
			for (j=0; j<6; j++ )	{
				bfwrite( &Segments[i].sides[j].wall_num, sizeof(short), 1, fp );
				bfwrite( &Segments[i].sides[j].tmap_num, sizeof(short), 1, fp );
				bfwrite( &Segments[i].sides[j].tmap_num2, sizeof(short), 1, fp );
			}
		}
	
	// Save the fuelcen info
		bfwrite( &Fuelcen_control_center_destroyed, sizeof(int), 1, fp );
		bfwrite( &Fuelcen_seconds_left, sizeof(int), 1, fp );
		bfwrite( &Num_robot_centers, sizeof(int), 1, fp );
		bfwrite( RobotCenters, sizeof(matcen_info)*Num_robot_centers, 1, fp );
		bfwrite( &ControlCenterTriggers, sizeof(control_center_triggers), 1, fp );
		bfwrite( &Num_fuelcenters, sizeof(int), 1, fp );
		bfwrite( Station, sizeof(FuelCenter)*Num_fuelcenters, 1, fp );
	
	// Save the control cen info
		bfwrite( &Control_center_been_hit, sizeof(int), 1, fp );
		bfwrite( &Control_center_player_been_seen, sizeof(int), 1, fp );
		bfwrite( &Control_center_next_fire_time, sizeof(int), 1, fp );
		bfwrite( &Control_center_present, sizeof(int), 1, fp );
		bfwrite( &Dead_controlcen_object_num, sizeof(int), 1, fp );
	
	// Save the AI state
		ai_save_state( fp );
	
	// Save the automap visited info
		bfwrite( Automap_visited, sizeof(ubyte)*MAX_SEGMENTS, 1, fp );
	}
	#endif
	bfwrite( &state_game_id, sizeof(uint), 1, fp );
	bfwrite( &Laser_rapid_fire, sizeof(int), 1, fp );
...
// Read description
	bfread( desc, sizeof(char)*DESC_LENGTH, 1, fp );

// Skip the current screen shot...
	bfseek( fp, THUMBNAIL_W*THUMBNAIL_H, SEEK_CUR );

// Read the Between levels flag...
	bfread( &between_levels, sizeof(int), 1, fp );

// Read the mission info...
	bfread( mission, sizeof(char)*9, 1, fp );
...
//Read level info
	bfread( &current_level, sizeof(int), 1, fp );
	bfread( &next_level, sizeof(int), 1, fp );

//Restore GameTime
	bfread( &GameTime, sizeof(fix), 1, fp );
...
//Read player info
	...
	if ( between_levels )	{
		//int saved_offset;
		bfread( &Players[Player_num], sizeof(player), 1, fp );
		//saved_offset = bftell(fp);
		//bfclose( fp );
		do_briefing_screens(next_level);
		//fp = bfopen( filename, "rb" );
		//bfseek( fp, saved_offset, SEEK_SET );
...
// Restore the weapon states
	bfread( &Primary_weapon, sizeof(byte), 1, fp );
	bfread( &Secondary_weapon, sizeof(byte), 1, fp );

	select_weapon(Primary_weapon, 0, 0, 0);
	select_weapon(Secondary_weapon, 1, 0, 0);

// Restore the difficulty level
	bfread( &Difficulty_level, sizeof(int), 1, fp );

// Restore the cheats enabled flag
	bfread( &Cheats_enabled, sizeof(int), 1, fp );
	bfread( &Game_turbo_mode, sizeof(int), 1, fp );
...
RetryObjectLoading:
		...
		bfread( &i, sizeof(int), 1, fp );
		Highest_object_index = i-1;
		if ( !BogusSaturnShit )	
			bfread( Objects, sizeof(object)*i, 1, fp );
		else {
			ubyte tmp_object[sizeof(object)];
			for (i=0; i<=Highest_object_index; i++ )	{
				bfread( tmp_object, sizeof(object)-3, 1, fp );
		...
		bfread( Walls, sizeof(wall)*Num_walls, 1, fp );
		// Check for a bogus Saturn version!!!!
		if (!BogusSaturnShit )	{
		...
		bfread( &i, sizeof(int), 1, fp );
		Num_open_doors = i;
		bfread( ActiveDoors, sizeof(active_door)*Num_open_doors, 1, fp );
	
		//Restore trigger info
		bfread( &Num_triggers, sizeof(int), 1, fp );
		bfread( Triggers, sizeof(trigger)*Num_triggers, 1, fp );
	
		//Restore tmap info
		for (i=0; i<=Highest_segment_index; i++ )	{
			for (j=0; j<6; j++ )	{
				bfread( &Segments[i].sides[j].wall_num, sizeof(short), 1, fp );
				bfread( &Segments[i].sides[j].tmap_num, sizeof(short), 1, fp );
				bfread( &Segments[i].sides[j].tmap_num2, sizeof(short), 1, fp );
			}
		}
	
		//Restore the fuelcen info
		bfread( &Fuelcen_control_center_destroyed, sizeof(int), 1, fp );
		bfread( &Fuelcen_seconds_left, sizeof(int), 1, fp );
		bfread( &Num_robot_centers, sizeof(int), 1, fp );
		bfread( RobotCenters, sizeof(matcen_info)*Num_robot_centers, 1, fp );
		bfread( &ControlCenterTriggers, sizeof(control_center_triggers), 1, fp );
		bfread( &Num_fuelcenters, sizeof(int), 1, fp );
		bfread( Station, sizeof(FuelCenter)*Num_fuelcenters, 1, fp );
	
		// Restore the control cen info
		bfread( &Control_center_been_hit, sizeof(int), 1, fp );
		bfread( &Control_center_player_been_seen, sizeof(int), 1, fp );
		bfread( &Control_center_next_fire_time, sizeof(int), 1, fp );
		bfread( &Control_center_present, sizeof(int), 1, fp );
		bfread( &Dead_controlcen_object_num, sizeof(int), 1, fp );
	
		// Restore the AI state
		ai_restore_state( fp );
	
		// Restore the automap visited info
		bfread( Automap_visited, sizeof(ubyte)*MAX_SEGMENTS, 1, fp );
	...
	if ( version >= 7 )	{
		int tmp_Lunacy;
		bfread( &state_game_id, sizeof(uint), 1, fp );
		bfread( &Laser_rapid_fire, sizeof(int), 1, fp );
		bfread( &Ugly_robot_cheat, sizeof(int), 1, fp );
		bfread( &Ugly_robot_texture, sizeof(int), 1, fp );
		bfread( &Physics_cheat_flag, sizeof(int), 1, fp );
		bfread( &tmp_Lunacy, sizeof(int), 1, fp );
		if ( tmp_Lunacy )
			do_lunacy_on();
	}
