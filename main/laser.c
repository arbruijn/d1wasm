...
int do_laser_firing(int objnum, int weapon_num, int level, int flags, int nfires)
{
	...
	switch (weapon_num) {
		...
		case VULCAN_INDEX: {
			...
			//if (psrand() > 24576)
			//	make_sound = 1;
			Laser_player_fire_spread( objp, VULCAN_ID, 6, psrand()/8 - 32767/16, psrand()/8 - 32767/16, make_sound, 0);
			if (nfires > 1) {
				Laser_player_fire_spread( objp, VULCAN_ID, 6, psrand()/8 - 32767/16, psrand()/8 - 32767/16, 0, 0);
				if (nfires > 2) {
					Laser_player_fire_spread( objp, VULCAN_ID, 6, psrand()/8 - 32767/16, psrand()/8 - 32767/16, 0, 0);
				}
			}
	...
	if (objp->id == SMART_ID) {
		...
		if (numobjs == 0) {
			...
			for (int i=0; i<NUM_SMART_CHILDREN; i++) {
				if (parent_type == OBJ_PLAYER) {
					int	hobjnum;
					hobjnum = create_homing_missile(objp, objlist[(psrand() * numobjs) >> 15].objnum, PLAYER_SMART_HOMING_ID, make_sound);
					// mprintf((0, "Object #%i is a PLAYER smart blob.\n", hobjnum));
				} else {
					int	hobjnum;
					hobjnum = create_homing_missile(objp, objlist[(psrand() * numobjs) >> 15].objnum, ROBOT_SMART_HOMING_ID, make_sound);
					// mprintf((0, "Object #%i is a robot smart blob.\n", hobjnum));
				}
				make_sound = 0;
			}
