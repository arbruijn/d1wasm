...
#define MENU_ORDER_GOG				26
#define MENU_ORDER_STEAM			27
#define MENU_ORDER_AD				28
#define MENU_CREDITS_GITHUB			29
...
extern void goto_steam(void);
extern void goto_gog(void);
extern void goto_github(void);
extern void goto_opl3(void);
extern void play_ad(void);
...
//temp!
		...
		case MENU_ORDER_GOG:
			goto_gog();
			break;
		case MENU_ORDER_STEAM:
			goto_steam();
			break;
		case MENU_ORDER_AD: {
			extern void play_stop();
			extern void songs_play(int, int);
			play_stop();
			play_ad();
			mprintf((0,"waked up"));
...
void do_order_menu()
{
	...
	do {
		...
		ADD_ITEM("BUY ON GOG", MENU_ORDER_GOG, -1 );
		ADD_ITEM("BUY ON STEAM", MENU_ORDER_STEAM, -1 );
		ADD_ITEM("DESCENT VS DOOM COMMERCIAL", MENU_ORDER_AD, -1 );
		ADD_ITEM("OK", -1, -1 );

		choice = newmenu_do2( NULL, "ORDERING INFO", num_options, m, NULL, choice, Menu_pcx_name );
...
void do_credits_menu()
{
	...
	//do {
		...
		ADD_ITEM("BROWSER PORT ON GITHUB", MENU_CREDITS_GITHUB, -1 );
		ADD_ITEM("OK", -1, -1 );

		choice = newmenu_do2( NULL, "CREDITS", num_options, m, NULL, choice, Menu_pcx_name );
		
		if ( choice > -1 && menu_choice[choice] > -1)
			do_option(menu_choice[choice]);
