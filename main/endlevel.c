...
void do_endlevel_frame()
{
	...
	if (!outside_mine) {
		...
		if ((explosion_wait1-=FrameTime) < 0) {
			...
			vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.rvec,(psrand()-PSRAND_MAX/2)*15);
			vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.uvec,(psrand()-PSRAND_MAX/2)*15);
	...
	if (Endlevel_sequence >= EL_FLYTHROUGH && Endlevel_sequence < EL_OUTSIDE)
		if ((explosion_wait2-=FrameTime) < 0) {
			...
			vm_vec_copy_scale(&tpnt,&ConsoleObject->orient.rvec,(psrand()-PSRAND_MAX/2)*100);
			vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.uvec,(psrand()-PSRAND_MAX/2)*100);
			vm_vec_add2(&tpnt,&ConsoleObject->pos);

			if (Endlevel_sequence == EL_FLYTHROUGH)
				vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.fvec,psrand()*200);
			else
				vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.fvec,psrand()*60);
			...
			if (hit_data.hit_type==HIT_WALL && hit_data.hit_seg!=-1)
				object_create_explosion(hit_data.hit_seg,&hit_data.hit_pnt,i2f(3)+psrand()*6,VCLIP_SMALL_EXPLOSION);
