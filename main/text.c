...
void load_text()
{
	...
	int n = N_TEXT_STRINGS;
	#ifdef SHAREWARE
	n = 514;
	Text_string[518] = strdup("ALT-F2\t  Save Game");
	Text_string[519] = strdup("ALT-F3\t  Load Game");
	Text_string[520] = "Only in Registered version!";
	#endif
	for (i=0,tptr=text;i<n;i++) {
		char *p;
		...
		if (!tptr)
			Error("Not enough strings in text file - expecting %d, found %d",n,i);
